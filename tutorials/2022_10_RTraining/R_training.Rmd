...
Now, select the line (or put your cursor on it) and press `Run`. You can also use `Ctrl + enter`.
...
The symbol `#` is used for **comments** in the code. This is a recommended for organization and helps you remember later what you were thinking when you wrote the code. It is also a good practice to help others reading and understanding your code. For example:
...
**Important**: when you want to alter your code, edit your **script** and do not type directly on the console, as the latter cannot be saved!
...
Another reccomendation in R is to save the script in the same folder as your raw data (input files) and the processed data (plots, tables and other output files). Then, we need to identify the folder as the **working directory**, making R understand that files will be read from there and files can be saved on that folder.
...
## Making live easier - `Tab`

Now imagine that you have a path to a folder like `~/Documentos/mestrado/semestre1/disciplina_tal/aula_tal/dados_28174/analise_276182/resultados_161/`. It is difficult to remember this long path in `setwd()`.

In addition to the RStudio `File` tab, you can also use `Tab` to help in completing the path. Try to use that key to explore some of your folders. Just start to type and press `Tab`! If you have two files or folders starting with the same pattern, press `Tab` twice to see the options.
...
First of all, R can be used as a simple calculator, having the same syntax as other softwares :
...
Another common mistake is to forget to close the parentheses, quotes, brackets or braces, making R to wait for the block to be closed. It sinalizes with an `+`:
...
Vectors are the simplest data structures on R. To build a vector with a numeric sequence we can use the following syntax:
...
Storing information in objects and their possible manipulation make R a *object oriented* language. To create an object, just assign **values** to **variables**:
...
The symbol of assignment `<-` is common among old programmers, but it has the same meaning as `=`. Some people prefer  `<-` for assignment and `=` to define arguments within functions. Feel free to organize your code in the most confortable way for you.
...
For more than one vector, R will align them and perform the operation element by element:
...
Multiple positions can be accessed. For example, let's check the positions 2 to 4:
...
## Outher vector categories

Vectors can also store other kind of data, as **characters**:

```{r}
clone <- c("GRA02", "URO01", "URO03", "GRA02", "GRA01", "URO01")
```

Another class is **factors**, that are values categorized in `levels`. Let's see what happens when we transform the character vector `clone` into a factor. It will assign levels to each word:
...
They are commonly used to identify positions of interest. In our case, let's see which are the `x` elements greater then 40:
...
Create a numeric sequence containing 10 values and save in a object defined as `a`.
...
Matrices are another class of objects in R and are important for large-scale operation. For this reason, they normally store numeric elements. To create a matrix, we enter a sequence of numbers and inform the number of columns and the number of rows:
...
Using these operations requires knowledge in matrix algebra and if you want to know more, *Linear Models in Statistics, Rencher (2008)* has a good review on these topics. You can also explore the R syntax for that [link](http://www.statmethods.net/advstats/matrix.html).
...
Different from matrices, we do not perform mathematical operations with data frames, but they allow the joining of vectors with different classes.  They are similar to tables generated in other software, as Excel.

Os *data frames* são combinações de vetores de mesmo comprimento. Todos os vetores que criamos até agora tem tamanho 6, verifique. Podemos assim combiná-los em colunas de um único data.frame:
...
* Using the diameter and height, calculate the volume according to the formulae below. Save the result in an object named `volume`:
...
```{r}
my_listt <- list(field1 = field1, mean_height = tapply(field1$height, field1$age, mean), matrix_ex = W)
str(my_listt)
```

To access the `campo1` data frame:
...
Lists are extremely useful when we pretend to use or create many objects in a loop. 
...
The RData files are exclusive to R. To load the objects saved on a RData:

```{r}
load("day1.RData")
```

It will make R recover all objects that we created up to now. To create this file:

```{r}
save.image(file = "day1.RData")
```

This command saves every object present in the global environment (top right corner). To save just one object the command is `save`:
...
```{r}
coffee_data <- read.table("data/FAOSTAT_Production_2019.csv",
...
We notice that some column names have not the best format. We can change it:
...
Now we will use this data to learn about different commands and functions in R.
...
Our next activity with the coffee data set is to understand the `if` and `else` structures. Basically, we establish a condition in the `if` structure and, if true, we are going to perform an activity. If the condition is false, we can include a task in the `else` part:
...
In the cases above, `i` works as an index that is going to assume the values from 1 to 10 and we have an operation within the braces.
...
The RStudio script editor helps with indentation. If you copy a code and it is not indented as expected, select the lines and press `Ctrl+i`.

We can use this structure to identify patterns in our dataset. For example, we can substitute the regions identified as Australia and them correct them to Oceania.
...
It is important that this condition is met otherwise the loop will work infinitely. If that happens, you will use other ways to finish the process: by pressing the "STOP" button in RStudio or by `Ctrl+C`.
...
To increase the fun, it is possible to use the loop structures inside other loops. One example is to loop over the rows and columns of a matrix:
...
* The conditional and loop structures have a proper syntax: If(){} e for(index in sequence){}. In the *for* loop we can change the name of the index, but we always need to define it and the sequence for iterating.
...
Now we can use the graphical functions available in R. In fact, R is really powerful in graphical terms and has very good packages to build graphics as `ggplot2`, `plotly` and `shiny`. They have a different syntax and will not be explored in this course.
