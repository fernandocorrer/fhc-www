...
To identify the pixels associated to the GCPs' coordinates, we will use WebODM. However, it only accepts PNG or JPG images. For that reason, we will choose one of the bands and convert it to PNG. On MacOS we can easily select images with a certain extension and convert them to the desired format. But we can make it simpler! We can install 
